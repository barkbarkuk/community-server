...
describe('An AcceptPreferenceParser', (): void => {
  ...
  const internal = { type: [{ value: 'internal/quads', weight: 0 }]};

  it('can handle all input.', async(): Promise<void> => {
    await expect(preferenceParser.canHandle({} as HttpRequest)).resolves.toBeUndefined();
  });

  it('always rejects internal/quads output.', async(): Promise<void> => {
    await expect(preferenceParser.handle({ headers: {}} as HttpRequest)).resolves.toEqual(internal);
  });
  ...
  it('parses accept headers.', async(): Promise<void> => {
    await expect(preferenceParser.handle({ headers: { accept: 'audio/*; q=0.2, audio/basic' }} as HttpRequest))
      .resolves.toEqual({ type: [
        { value: 'audio/basic', weight: 1 },
        { value: 'audio/*', weight: 0.2 },
        { value: 'internal/quads', weight: 0 },
      ]});
  });
  ...
  it('parses accept-charset headers.', async(): Promise<void> => {
    await expect(preferenceParser.handle(
      ...
      { charset: [{ value: 'iso-8859-5', weight: 1 }, { value: 'unicode-1-1', weight: 0.8 }], ...internal },
    );
  });
  ...
  it('parses accept-datetime headers.', async(): Promise<void> => {
    ...
    )).resolves.toEqual({ datetime: [{ value: 'Tue, 20 Mar 2001 20:35:00 GMT', weight: 1 }], ...internal });
  });
